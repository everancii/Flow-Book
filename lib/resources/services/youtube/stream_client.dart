...
class AudioStreamClient {
  static final AudioStreamClient _instance = AudioStreamClient._internal();
  final http.Client _httpClient = http.Client();

  factory AudioStreamClient() => _instance;

  AudioStreamClient._internal();
  ...
  Stream<List<int>> _getStream(
    ...
    while (currentChunkStart < end) {
      ...
      try {
        ...
        AppLogger.debug('Stream chunk timeout (attempt ${errorCount + 1})');
        if (errorCount >= 4) {
          rethrow;
        }
        errorCount++;
        ...
        AppLogger.debug('Stream chunk error (attempt ${errorCount + 1}): $e');
        if (errorCount >= 4) {
          rethrow;
        }
        errorCount++;
