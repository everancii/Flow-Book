  ...
  Future<void> downloadAudiobook(
    ...
    try {
      ...
      if (hasNotificationPermission) {
        _downloader.configureNotification(
          ...
          complete: TaskNotification(
              'Book downloaded: $audiobookTitle', 'File: {filename}'),
      ...
      for (int i = 0; i < files.length; i++) {
        ...
        if (url.isEmpty) {
          completedFiles++;
          totalProgress = completedFiles / totalFiles;
        ...
        String currentFileDirectoryPath =
            'downloads/${_safeDirectoryName(audiobookId)}';

        if (isYouTubeUrl) {
          File? outputFile;
          IOSink? fileStream;
          try {
            String? parsedVideoId = VideoId.parseVideoId(url);
  ...
  Future<void> _cleanupPartialDownload(String audiobookId) async {
    try {
      ...
      final downloadDir = Directory(
          '${baseDir.path}/downloads/${_safeDirectoryName(audiobookId)}');
...
String _safeDirectoryName(String id) =>
