...
class _HistorySectionState extends State<HistorySection> {
  ...
  String formatProgress(int position, double total, double completedTime) {
    ...
    if (totalMinutes >= 1000) {
      ...
      return '${completedMinutes.toString().padLeft(2, '0')}m / ${totalMinutes.toInt().toString().padLeft(2, '0')}m';
    }
  }
                      ...
                      onTap: () async {
                        final confirmed = await showDialog<bool>(
                          ...
                          builder: (context) => AlertDialog(
                            ...
                            content: Text(
                                'Are you sure you want to remove "${item.audiobook.title}" from recently played?'),
                            actions: [
                              TextButton(
  ...
  Widget _getOriginIcon(String? origin) {
    if (origin == 'librivox') {
      return const Icon(Icons.book, color: Colors.white, size: 15);
    } else if (origin == 'youtube') {
      return const Icon(Icons.ondemand_video, color: Colors.white, size: 15);
    } else if (origin == 'download') {
      return const Icon(Icons.cloud_download, color: Colors.white, size: 15);
    } else if (origin == 'local') {
      return const Icon(Icons.music_note, color: Colors.white, size: 15);
    } else {
      return const Icon(Icons.help_outline, color: Colors.white, size: 15);
    }
  }
